...
function openDraftModal(onPicked) {
  ...
  for (const cid of choices) {
    ...
    const choose = () => {
      ...
      // Keep master deck in sync so safety-rebuild preserves drafted cards
...
function drawHand(n) {
  ...
  for (let i = 0; i < n; i++) {
    if (state.drawPile.length === 0) {
      // Refill from discard first
      state.drawPile = shuffle(state.discardPile);
      state.discardPile = [];

      // Safety: if both piles are empty (can happen if the player rerolls/debug-picks without
      // properly discarding), rebuild from the master deck so the run never softlocks.
      if (state.drawPile.length === 0) {
        if (Array.isArray(state.masterDeck) && state.masterDeck.length) {
...
// Pick from a pool with graceful fallback when all weights collapse to 0.
// - First tries weighted selection.
// - If that fails, picks uniformly at random (preferably avoiding ids in avoidIds).
...
function pickDueStoryEvent(avoidIds = []) {
  ...
  let ev = pickEventFromPool(pool, hardAvoid, eventDirectorWeight, true);

  // If the pool is exhausted after avoiding repeats (tiny content sets), broaden the net.
  if (!ev) {
    let alt = eligibleEvents({ story: "exclude" });
    if (!alt.length) alt = eligibleEvents({});
    ev = pickEventFromPool(alt, hardAvoid, eventDirectorWeight, true);
  }

  // Absolute last resort: allow a repeat rather than stalling.
  if (!ev) ev = pickEventFromPool(pool, [], eventDirectorWeight, false);
  if (!ev) {
...
function tryPickStoryHookEvent(avoidIds = []) {
  ...
  return pickEventFromPool(pool, avoidIds, (ev) => {
...
function eligibleEvents(opts = {}) {
  ...
  return DATA.events.filter(e => {
    ...
    // Safety: never surface an event where every outcome is locked by requirements
    // (prevents "all options disabled" softlocks if authoring misses an event-level gate).
    const hasUnlockedOutcome = (e.outcomes ?? []).some(o => unmetReasons(o.requirements).length === 0);
    if (!hasUnlockedOutcome) return false;
...
function loadRandomEvent(opts = {}) {
  const avoidIds = Array.isArray(opts.avoidIds) ? opts.avoidIds : [];
  // Always avoid immediate repeats (plus any explicit avoids from rerolls).
  const hardAvoid = Array.from(new Set([
    ...avoidIds,
    ...(state?.history?.recentEvents?.slice(0, 1) ?? [])
  ]));
  ...
  if (majorBeat) {
    ...
    // If avoiding repeats exhausts the pool (common early while content is thin),
    // broaden to non-story general events instead of repeating the same major.
    if (!ev) {
      let alt = eligibleEvents({ kind: "general", story: "exclude" });
      if (!alt.length) alt = eligibleEvents({ story: "exclude" });
      ev = pickEventFromPool(alt, hardAvoid, eventDirectorWeight, true);
    }
  ...
  const due = pickDueStoryEvent(hardAvoid);
  if (due) {
    beginEvent(due);
    return;
  }
  ...
  if (forceHook || Math.random() < hookChance) {
    const hook = tryPickStoryHookEvent(hardAvoid);
    if (hook) {
      beginEvent(hook);
      return;
    }
  ...
  const ev = pickEventFromPool(pool, avoidIds, eventDirectorWeight, true);
  if (!ev) {
    console.warn("No eligible events found for current age/filters.");
    return;
  }
